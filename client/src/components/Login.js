    ...
    return (
      <div className="login-form-container">
        <form onSubmit={this.handleSubmit} className="login-form">
          ...
          <button type="submit">{this.state.login ? 'Log in' : 'Sign up'}</button>
        </form>
      <button onClick={this.handleClick}>{this.state.login ? "Don't have an account? Sign up" : "Already have an account? Log in"}</button>
      </div>
    )
  }
}
